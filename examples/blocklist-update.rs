...
use transmission_rpc::types::{BasicAuth, BlocklistUpdate, Result, RpcResponse, Tag};
...
async fn main() -> Result<()> {
    ...
    let tag = Tag(-123);
    let response: Result<RpcResponse<BlocklistUpdate>> = client.blocklist_update_tagged(tag).await;
    match &response {
        Ok(resp) => {
