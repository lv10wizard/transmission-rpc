...
fn strip_serde_attr(serde_attrs: Vec<Attribute>, to_strip: &str) -> Result<Vec<Attribute>> {
    ...
    for attr in serde_attrs.into_iter() {
        ...
        stripped_attrs.push(match needs_new_attr {
            true => {
...
///
/// If `version >= Version::new(6, 0, 0)`, the `rename = "..."` serde argument will be stripped out
/// if it was defined. This behavior assumes that any `rename = "..."` serde argument is used to
/// override container-level `rename_all = "..."` behavior, eg. `rename = "file-count"` to override
/// `camelCase`. Starting with semver-6.0.0, all of these serialized strings should be uniformly
/// `snake_case`d.
