        ...
        match self.jsonrpc.as_ref() {
            Some(jsonrpc) => {
                JsonRpcRequest {
                    ...
                    /*
                    params: self.arguments.map(|args| match args {
                        Args::FreeSpace(x) => x.into_compat(),
...
/*
impl Args {
    fn into_compat<T>(self) -> T {
        match self {
            Args::FreeSpace(x) => x.into_compat(),
            Args::GroupGet(x) => x.into_compat(),
            Args::GroupSet(x) => x.into_compat(),
            Args::SessionGet(x) => x.into_compat(),
            Args::SessionSet(x) => x.into_compat(),
            Args::QueueMove(x) => x.into_compat(),
            Args::TorrentGet(x) => {
                TorrentGetArgs {
                    fields: x.fields
                        .map(|fields| fields.into_iter().map(|f| ccase!(snake, f))),
                    ids: x.ids,
                }
            },
            Args::TorrentAction(x) => x.into_compat(),
            Args::TorrentRemove(x) => x.into_compat(),
            Args::TorrentAdd(x) => x.into_compat(),
            Args::TorrentSet(x) => x.into_compat(),
            Args::TorrentSetLocation(x) => x.into_compat(),
            Args::TorrentRenamePath(x) => x.into_compat(),
        }
    }
}
*/
